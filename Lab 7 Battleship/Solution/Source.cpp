...
void playerShips(char friendlyboard[][10], char enemyboard[][10]) //Places the player's ships.
{
	int direction;
	ship fFleet[5];
	...
	for (int i = 4; i >= 0; i--) /*Puts restrictions on where ships can be placed. Unfortunantly the parts intended to prevent
								 ships from overlapping simply didn't work.*/
	{
		...
		cout << "What direction would you like the ship to point?" << endl; //Only prompts a given direction if far enough from the edge.
...
void enemyShips(char friendlyboard[][10], char enemyboard[][10]) /*Places the enemy ships. Almost identical to playerShips
																 except with random numbers rather than inputted ones.*/
{
...
int playerAttack(char friendlyboard[][10], char friendlytarget[][10], char enemyboard[][10]) /*Allows the player to attack and responds
																							 with the result of that attack.*/
{
	int targetX;
	int targetY;
	cout << "It is your turn. To target a postion, first enter its collumn, then its row.\n\n";
	cin >> targetX >> targetY;
	while (enemyboard[targetX - 1][targetY - 1] == 'H' || enemyboard[targetX - 1][targetY - 1] == 'M' //Stops attacks outside the board
		|| targetX > 10 || targetX < 1 || targetY > 10 || targetY < 1) //Stops attack on already attacked tiles.
	{
	...
	for (int j = 0; j < 10; j++) //Checks if there are any ship (#) tiles left on the board.
	{
		for (int i = 0; i < 10; i++)
		{
			if (enemyboard[j][i] == '#')
...
int enemyAttack(char friendlyboard[][10], char friendlytarget[][10], char enemyboard[][10]) /*Allows the enemy to attack. Almost 
																							identical to playerAttack except with random
																							numbers rather than user inputs.*/
{
